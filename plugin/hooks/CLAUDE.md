...
SessionStart, PreCompact, PreModelSwitch, Stop, SubagentStop,
...
| `tkr hook pre-model-switch` (native Go verb, no JS) | PreModelSwitch | Precise prefix-cache-cost warning on a model switch (issue #981) — a switch unconditionally busts the cache, unlike an edit which only maybe does. Reads `context_tokens`/`estimated_cache_write_usd` off the payload; two independent, OR'd, configurable threshold pairs (`model_switch_warn_{tokens,usd}`, `model_switch_block_{tokens,usd}`, `internal/config`): warn prints a user-facing `systemMessage`, block returns `permissionDecision:"ask"` (confirmed interactively, refused when there's no terminal to ask — a client/API-driven switch). `model_switch_warn_enabled=false` or `TKR_PRE_MODEL_SWITCH_DISABLED=1` disables the hook entirely. No JS predecessor — new in this port generation. Implementation is `internal/hooks/premodelswitch.go` |
